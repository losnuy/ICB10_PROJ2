...
class NaverApiClient:
    ...
    def _handle_response(self, response: requests.Response) -> dict:
        ...
        if status == 400:
            st.error(f"잘못된 요청 파라미터입니다. (400 Bad Request)\n상세 내용: {err_msg}")
        elif status == 401:
            st.error("API 인증에 실패했습니다. 사이드바에 입력된 Client ID 및 Client Secret이 유효한지 확인해 주세요. (401 Unauthorized)")
        elif status == 403:
            st.error("API 권한이 없습니다. 네이버 개발자 센터에서 해당 API(데이터랩, 검색 등)가 애플리케이션 사용 API로 활성화되어 있는지 확인해 주세요. (403 Forbidden)")
        elif status == 429:
            st.error("API 일일 호출 한도를 초과했습니다. 내일 다시 시도해 주세요. (429 Too Many Requests)")
        elif status == 500:
            st.error(f"네이버 서버 내부 에러가 발생했습니다. 잠시 후 다시 시도해 주세요. (500 Internal Server Error)\n상세 내용: {err_msg}")
        else:
            st.error(f"에러가 발생했습니다. (HTTP {status})\n상세 내용: {err_msg}")
